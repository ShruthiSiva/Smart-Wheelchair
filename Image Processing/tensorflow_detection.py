...
MODEL_NAME = 'Disability_logo_graph'
...
def visualize_boxes_and_labels_on_image_array(image,
  ...
  k=[(xmax-xmin)*width, (ymax-ymin)*height]
...
TEST_IMAGE_PATHS = [ os.path.join(PATH_TO_TEST_IMAGES_DIR, 'image{}.jpg'.format(i)) for i in range(15, 20) ]
...
with detection_graph.as_default():
  with tf.Session(graph=detection_graph) as sess:
    ...
    for image_path in TEST_IMAGE_PATHS:
      ...
      image_np, k = visualize_boxes_and_labels_on_image_array(
          image_np,
          np.squeeze(boxes),
